...
class Environment
{
	...
	/**
	 * Detect mode by domain name
	 * @return string detected mode
	 */
	protected function getModeByServerName() {
		$mode = 'DEVELOPMENT';
		if(!empty($_SERVER['SERVER_NAME'])){
			switch ($_SERVER['SERVER_NAME']) {
				case 'kyotokimono-rental.com':
					$mode = 'PRODUCTION';	
					break;
	...
	{
		if ($this->mode=='PRODUCTION' && $_SERVER['REMOTE_ADDR'] !='115.79.33.251' &&  $_SERVER['REMOTE_ADDR']!='113.172.147.24' ) return;
		
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') return; // if ajax
			
		echo '<div id="edebug" style="display:none ;position: ; bottom: 0; left: 0; z-index: 99999; height: 250px; overflow: auto; background-color: #ddd; color: #000; border: 1px solid #000; margin: 5px; padding: 5px;">
			<!--<pre>'.htmlspecialchars(print_r($this, true)).'</pre>--></div>'
				. '<script>document.getElementById("edebug").style.display = "";'
				. 'document.getElementById("edebug").innerHTML = ((document.getElementById("edebug").innerHTML).replace(/<!--/,"")).replace(/-->/,"");</script>';
	}
